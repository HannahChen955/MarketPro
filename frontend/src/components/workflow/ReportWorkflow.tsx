...
import { DataCollectionForm, DataCollectionFormData } from './DataCollectionForm';
import { ProgressTracker, defaultProgressStages, ProgressStage } from './ProgressTracker';
import { AIAssistant } from '@/components/chat/AIAssistant';
import { ReportPreview } from '@/components/preview/ReportPreview';
import { cn } from '@/lib/utils';

export interface ReportWorkflowProps {
  reportTypeId: string;
...
export function ReportWorkflow({
  ...
  // 处理表单提交
  const handleFormSubmit = (data: DataCollectionFormData) => {
    setState(prev => ({
      ...prev,
      formData: data,
      currentStep: 1
    ...
    <div className={cn('min-h-screen bg-gray-50', className)}>
      <Container className="py-8">
        ...
        <div className={cn(
          ...
          <div className={cn(
            'flex-1',
            (state.showAIAssistant || state.showPreview) && state.previewMode === 'side' ? 'lg:col-span-8' : 'w-full'
          )}>
            {/* 工作流程步骤 */}
            <Card className="mb-8">
